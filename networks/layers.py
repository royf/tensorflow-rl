...
def conv2d(name, vd, _input, filters, size, channels, stride, activation='relu', padding='VALID', data_format='NHWC'):
    ...
    def mymap(fn, inp, wei):
        indices = tf.range(tf.shape(inp)[0])
        out = tf.map_fn(lambda ii: fn(inp[ii:ii+1], wei[ii]), indices, dtype=tf.float32)
        return out

    conv = mymap(lambda inp, wei: tf.nn.conv2d(inp, wei, strides=strides,
